#!/usr/bin/env bash
# Phase 42B-FIX — production deploy: Global Consistency Guard bundle
set -euo pipefail

APP=/opt/worldcup-predictor
FRONTEND=/var/www/worldcup/frontend/dist
TS=$(date -u +%Y%m%d-%H%M%S)
BACKUP="${APP}/backups/deploy-phase42b-fix-${TS}"
TARBALL="${1:-/tmp/phase42b_fix_deploy.tar.gz}"

echo "=== Phase 42B-FIX Production Deploy (Consistency Guard Bundle) ==="
echo "Backup: ${BACKUP}"
mkdir -p "${BACKUP}"
...
tar czf "${BACKUP}/repo_snapshot_pre.tar.gz" \
  ...
  worldcup_predictor/prediction/market_consistency_config.py \
...
sudo -u www-data env PYTHONPATH="${APP}" APP_ENV=production bash -lc \
  "cd ${APP} && set -a && source .env.production && set +a && \
   .venv/bin/python scripts/validate_phase42b_final_consistency_audit.py && \
   .venv/bin/python scripts/validate_phase42b_global_market_consistency_guard.py && \
   .venv/bin/python scripts/validate_bugfix_timing_range_consistency.py" \
  2>&1 | tee "${BACKUP}/validate_42b_fix.log" | tail -30

echo "=== 5. Smoke ==="
bash scripts/deploy_phase42b_fix_smoke.sh 2>&1 | tee "${BACKUP}/smoke.log"
