...
class BillingService:
    ...
    def plan_to_price_id(self, plan: str) -> str:
        ...
        if not price_id:
            raise PlanPriceMappingError(
                f"Stripe price not configured for plan: {key}",
    ...
    def readiness(self) -> BillingReadinessResponse:
        ...
        checkout_enabled = configured and self._client.sdk_ready()
        return BillingReadinessResponse(
            ...
            portal_enabled=(
                configured
                and self._client.sdk_ready()
                and s.stripe_portal_return_url_configured
            ),
            message=None if checkout_enabled else "Payment checkout is not active yet.",
        )
    ...
    def create_checkout_session(
        ...
        *,
        ...
        if not readiness.checkout_enabled:
            raise CheckoutValidationError("Checkout is not enabled", code="checkout_disabled", status_code=503)
