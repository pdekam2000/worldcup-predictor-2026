...
def main() -> int:
    ...
    record("plan_rank_helpers", all(x in pricing for x in ("normalizePlanKey", "canUpgradeTo", "isPremiumPlan")))
    record("subscription_premium_active_badge", "Premium Active" in sub_page)
    record("subscription_current_plan_disabled", "Current Plan" in sub_page and "canUpgradeTo" in sub_page)
    record("subscription_included_tier_guard", "Included" in sub_page and "isIncludedTier" in sub_page)
    record("upgrade_dialog_blocks_lower_tier", "upgradeAllowed" in upgrade and "canUpgradeTo" in upgrade)
    record("upgrade_dialog_no_checkout_when_blocked", "if (!upgradeAllowed || !checkoutConfigured) return" in upgrade)
    record("checkout_inactive_message", "Payment checkout is not active yet." in upgrade)
    record("saas_api_detail_message_parse", "detail.message" in saas_api)
