...
from worldcup_predictor.billing.user_messages import CHECKOUT_INACTIVE_MSG, message_for_code
...
def _legacy_checkout_payload(svc: BillingService) -> dict[str, Any]:
    ...
    return {
        ...
        "message": readiness.message or (CHECKOUT_INACTIVE_MSG if not configured else None),
    }
...
def create_checkout_session(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={"message": message_for_code(exc.code, str(exc)), "code": exc.code},
        ) from exc
