...
def build_accuracy_optimization_report(
    *,
    ...
    insufficient = agent_baseline.settled < 20
    ...
    return {
        ...
        "trust_message": (
            "Learning insights require at least 20 evaluated real predictions."
            if insufficient
            else None
        ),
        "sample_size": {
        ...
        "best_markets": best_markets if not insufficient else [],
        "worst_markets": worst_markets if not insufficient else [],
        "best_confidence_buckets": best_buckets if not insufficient else [],
        "worst_confidence_buckets": worst_buckets if not insufficient else [],
        "top_agents": [a for a in agent_analysis if a.get("winrate") is not None][:5] if not insufficient else [],
        "weakest_agents": sorted(
            [a for a in agent_analysis if a.get("winrate") is not None],
            key=lambda x: x.get("winrate") or 0,
        )[:5] if not insufficient else [],
        "improvement_suggestions": suggestions if not insufficient else [],
        "insights": insights,
    }
