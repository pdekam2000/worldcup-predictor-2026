...
def render_odds_api_dev_status(locale: Locale) -> None:
    """Developer API page — Odds API connection, quota, last skip reason."""
    settings = get_settings()
    summary = usage_summary(settings)
    with st.container(border=True):
        st.markdown(f"**{gui_t('odds_api.panel_title', locale)}**")
        configured = settings.the_odds_api_configured
        st.caption(
            f"{gui_t('odds_api.configured', locale)}: **{'✓' if configured else '✗'}**"
        )
        if not configured:
            st.info(gui_t("odds_api.not_configured", locale))
            return
        c1, c2, c3 = st.columns(3)
        with c1:
            st.metric(gui_t("odds_api.used_today", locale), summary["daily_used"], f"/ {summary['daily_hard_limit']}")
        with c2:
            st.metric(gui_t("odds_api.used_month", locale), summary["monthly_used"], f"/ {summary['monthly_limit']}")
        with c3:
            st.metric(gui_t("odds_api.remaining_month", locale), summary["monthly_remaining"])
        guard_ok = summary["daily_used"] < summary["daily_hard_limit"]
        st.caption(
            f"{gui_t('odds_api.guard_status', locale)}: "
            f"**{'OK' if guard_ok else gui_t('odds_api.guard_blocked', locale)}**"
        )
        last = st.session_state.get("odds_api_last_skip_reason")
        if last:
            st.caption(gui_t("odds_api.last_skipped", locale).format(status=last))
    ...
    with st.container(border=True):
        ...
        if guard:
            ...
            if guard.get("used_live"):
                ...
                st.session_state["odds_api_last_skip_reason"] = _status_label(reason, locale)
