...
def validate_role_change(
    *,
    ...
    if target.role == UserRole.OWNER and new_role != UserRole.OWNER:
        raise UserManagementError("Owner role cannot be changed via admin API.", status_code=409)
    if target.role == UserRole.SUPER_ADMIN and new_role not in (UserRole.SUPER_ADMIN, UserRole.OWNER):
