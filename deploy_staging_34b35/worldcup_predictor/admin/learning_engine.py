...
def build_learning_dashboard(
    *,
    ...
    recommendations = _recommendations(agent_metrics, market_metrics, confidence_metrics)

    top_agents = [a for a in agent_metrics if a.get("winrate") is not None][:5]
    worst_agents = sorted(
        [a for a in agent_metrics if a.get("winrate") is not None],
    ...
    base = {
        ...
        "best_markets": [m for m in market_metrics if m.get("winrate") is not None][:5],
