#!/usr/bin/env bash
# Hotfix Pack 3 — production deploy (evaluated visibility)
set -euo pipefail
APP=/opt/worldcup-predictor
TS=$(date -u +%Y%m%d-%H%M%S)
BACKUP="${APP}/backups/hotfix-pack3-${TS}"
TARBALL="${1:-/tmp/hotfix_pack3_deploy.tar.gz}"
...
sed -i 's/\r$//' scripts/_remote_deploy_hotfix_pack3.sh scripts/validate_hotfix_pack3_evaluated_visibility.py 2>/dev/null || true
bash scripts/_remote_deploy_hotfix_pack3.sh "${BACKUP}"
