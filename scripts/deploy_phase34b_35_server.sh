...
echo "=== Phase 36C env wiring validation ==="
...
_run_validation scripts/validate_phase34b_stale_confidence_cache_fix.py 2>&1 | tee "${BACKUP}/validate_phase34b.log" | tail -8

echo "=== Phase 36B placeholder repair validation ==="
_run_validation scripts/validate_phase36b_placeholder_repair.py 2>&1 | tee "${BACKUP}/validate_phase36b.log" | tail -10

echo "=== Phase 35 validation ==="
_run_validation scripts/validate_phase35_accuracy_driven_optimization.py 2>&1 | tee "${BACKUP}/validate_phase35.log" | tail -8

echo "=== Phase 33 regression ==="
_run_validation scripts/validate_phase33_background_prediction_evaluation.py 2>&1 | tee "${BACKUP}/validate_phase33.log" | tail -5

echo "=== Phase 33B regression ==="
_run_validation scripts/validate_phase33b_no_bet_ux_replacement.py 2>&1 | tee "${BACKUP}/validate_phase33b.log" | tail -5

echo "=== Phase 34 regression ==="
_run_validation scripts/validate_phase34_admin_accuracy_learning_subscription.py 2>&1 | tee "${BACKUP}/validate_phase34.log" | tail -5
