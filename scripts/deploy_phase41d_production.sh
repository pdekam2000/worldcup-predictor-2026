#!/usr/bin/env bash
# Phase 41D — production deploy: Change Password in Settings
set -euo pipefail

APP=/opt/worldcup-predictor
FRONTEND=/var/www/worldcup/frontend/dist
TS=$(date -u +%Y%m%d-%H%M%S)
BACKUP="${APP}/backups/deploy-phase41d-${TS}"
TARBALL="${1:-/tmp/phase41d_deploy.tar.gz}"

echo "=== Phase 41D Production Deploy (Change Password) ==="
echo "Backup: ${BACKUP}"
mkdir -p "${BACKUP}"
...
tar czf "${BACKUP}/repo_snapshot_pre.tar.gz" \
  worldcup_predictor/auth/change_password.py \
  worldcup_predictor/api/routes/auth.py \
  scripts/validate_phase41d_change_password.py \
  2>/dev/null || true
...
echo "=== 4. Validation ==="
_run_validation scripts/validate_phase41d_change_password.py 2>&1 | tee "${BACKUP}/validate_41d.log" | tail -30 || true

echo "=== 5. Smoke tests ==="
bash scripts/deploy_phase41d_smoke.sh 2>&1 | tee "${BACKUP}/smoke.log"
