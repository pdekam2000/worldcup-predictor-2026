#!/usr/bin/env python3
"""Controlled 1. Deild Tier B pilot validation — evidence collection."""
...
ART = ROOT / "artifacts" / "one_deild_tier_b_pilot"
...
def main() -> int:
    ...
    from worldcup_predictor.gpt_actions.delegation import _match_odds, discover_today_matches
    ...
    out["identity_confirmed"] = (
        ...
        and str(league_identity.get("name") or "") == "1. Deild"
        and str(league_identity.get("country") or "") == "Iceland"
    )

    meta = get_tier_b_domain(CANONICAL_KEY)
    out["registry_entry"] = meta
    out["one_lyga_preserved"] = get_tier_b_domain("one_lyga") is not None
            ...
            prematch.append(
                {
                    ...
                    "bookmaker_count": bk,
                    "api_odds_present": api_bk > 0,
                    "db_odds_present": db_bk > 0,
                }
            )
    ...
    bks = [f["bookmaker_count"] for f in with_odds]
    out["fixture_volume"] = {
        ...
        "fixtures_with_any_odds": len(with_odds),
        "median_bookmakers": sorted(bks)[len(bks) // 2] if bks else 0,
    ...
    out["broad_listing"] = {
        "date": PILOT_DATE,
        "one_deild_count": len(one_deild_broad),
        "samples": one_deild_broad[:5],
    }
    ...
    out["discover"] = {
        ...
        "owner_one_deild": [
            m
            for m in discover_owner.get("matches") or []
            if m.get("competition") == CANONICAL_KEY
        ],
    }

    control = next(
        (f for f in prematch if f["date"] == PILOT_DATE and f["bookmaker_count"] > 0),
        next((f for f in prematch if f["date"] == PILOT_DATE), None),
    )
    if control is None and prematch:
        control = next((f for f in prematch if f["bookmaker_count"] > 0), prematch[0])
    ...
    if control:
        ...
        gate_status, gate_detail = classify_candidate(
            conn,
            fixture={
                **control,
                **unified,
                "competition": comp,
                "competition_raw": f"league_{PROVIDER_LEAGUE_ID}",
                "tier": "B",
            },
            settings=settings,
        )
        ...
        if odds_meta.get("bookmaker_count", 0) > 0:
            try:
                ...
                wde_status, wde_payload = run_daily_wde(
                    prepared,
                    repo=repo,
                    settings=settings,
                    conn=conn,
                ...
                wde_result["executed"] = wde_status in ("generated", "dry_run")
                wde_result["status"] = wde_status
                wde_result["decision"] = wde_payload.get("predicted_1x2") or wde_payload.get("prediction")
        ...
        if odds_meta.get("bookmaker_count", 0) > 0:
            try:
                ...
                ecse_status, ecse_payload = run_daily_ecse(
                    prepared,
                    settings=settings,
                    conn=conn,
                    dry_run=False,
                ...
                ecse_result = {
                    "executed": ecse_status in ("generated", "dry_run"),
                    "status": ecse_status,
        ...
        json.dumps(
            {
                "status": "ok",
                "path": str(path),
                "identity": out["identity_confirmed"],
                "gate": out.get("fixture_gate", {}).get("status"),
                "odds_books": (out.get("odds") or {}).get("bookmaker_count"),
            }
        )
    )
