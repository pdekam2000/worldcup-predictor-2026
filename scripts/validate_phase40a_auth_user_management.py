        ...
        if not postgres_configured():
            ...
            with saas_uow() as uow:
                ...
                if user:
                    ...
                    raw, _delivery = issue_verification_token(user.id, email=user.email)
            ...
            if settings.super_admin_access_key:
                ...
                admin_gate_token = None
                if settings.admin_access_key:
                    ok_admin, _, _, admin_gate_token = attempt_gate_unlock(
                        user_id=str(owner.id),
                        gate="admin",
                        access_key=settings.admin_access_key,
                    )
                    if ok_admin and admin_gate_token:
                        super_headers["X-Admin-Gate-Token"] = admin_gate_token
