...
def register(body: RegisterRequest, request: Request) -> RegisterPendingResponse:
    ...
    write_auth_audit_event("register_success", user_id=profile.id, email=profile.email, ip=ip)
...
def resend_verification(body: ResendVerificationRequest, request: Request) -> MessageResponse:
    resend_verification_for_email(body.email.strip().lower())
    write_auth_audit_event("verification_resent", email=body.email.strip().lower(), ip=_client_ip(request))
