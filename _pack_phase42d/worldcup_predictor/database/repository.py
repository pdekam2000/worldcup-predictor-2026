...
class FootballIntelligenceRepository:
    ...
    # ------------------------------------------------------------------ #
    # Phase 32C — national team history caches
    # ------------------------------------------------------------------ #
    ...
    def upsert_team_mapping(
        ...
        *,
        ...
        self._conn.execute(
            """
            INSERT INTO teams (api_team_id, name, competition_key)
            VALUES (?, ?, ?)
            ON CONFLICT(api_team_id, competition_key) DO UPDATE SET
                name = excluded.name
            """,
            (api_team_id, name, competition_key),
        )
        self._conn.commit()
    ...
    def lookup_team_id_by_name(
        ...
        *,
        ...
        if not target:
            return None
        rows = self._conn.execute(
            "SELECT api_team_id, name FROM teams WHERE competition_key = ?",
            (competition_key,),
        ).fetchall()
        for row in rows:
            stored = normalize_team_name(str(row["name"]))
            if stored == target or target in stored or stored in target:
                return int(row["api_team_id"])
        return None

    def save_fixture_team_resolution(
        self,
        fixture_id: int,
        *,
        ...
        self._conn.execute(
            """
            INSERT INTO fixture_team_resolution (
                fixture_id, home_team_id, away_team_id, resolution_source, updated_at
            ) VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(fixture_id) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
            """,
            (fixture_id, home_team_id, away_team_id, resolution_source, _utc_now()),
        )
        self._conn.commit()

    def get_fixture_team_resolution(self, fixture_id: int) -> dict[str, Any] | None:
        row = self._conn.execute(
            "SELECT * FROM fixture_team_resolution WHERE fixture_id = ?",
            (fixture_id,),
        ).fetchone()
        return dict(row) if row else None

    def save_national_form_cache(
        self,
        *,
        team_id: int,
        team_name: str,
        ...
        self._conn.execute(
            """
            INSERT INTO national_team_form_cache (
                team_id, team_name, matches_used, last5_json, last10_json,
                home_json, away_json, neutral_json, recent_fixtures_json,
                national_form_score, explanation_json, source, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(team_id) DO UPDATE SET
                team_name = excluded.team_name,
                matches_used = excluded.matches_used,
                last5_json = excluded.last5_json,
                last10_json = excluded.last10_json,
                home_json = excluded.home_json,
                away_json = excluded.away_json,
                neutral_json = excluded.neutral_json,
                recent_fixtures_json = excluded.recent_fixtures_json,
                national_form_score = excluded.national_form_score,
            ...
            (
                team_id,
                team_name,
                int(payload.get("matches_used") or 0),
                json.dumps(payload.get("last5"), ensure_ascii=False),
                json.dumps(payload.get("last10"), ensure_ascii=False),
                json.dumps(payload.get("home"), ensure_ascii=False),
                json.dumps(payload.get("away"), ensure_ascii=False),
                json.dumps(payload.get("neutral"), ensure_ascii=False),
                json.dumps(payload.get("recent_fixtures"), ensure_ascii=False),
                payload.get("national_form_score"),
                json.dumps(payload.get("explanation") or [], ensure_ascii=False),
                str(payload.get("source") or "cache_backfill"),
                _utc_now(),
            ),
        )
        self._conn.commit()

    def get_national_form_cache(self, team_id: int) -> dict[str, Any] | None:
        row = self._conn.execute(
            "SELECT * FROM national_team_form_cache WHERE team_id = ?",
            (team_id,),
        ).fetchone()
        if not row:
            return None
    ...
    def save_national_h2h_cache(
        ...
        *,
        ...
        pair_key = f"{min(home_team_id, away_team_id)}-{max(home_team_id, away_team_id)}"
        self._conn.execute(
            """
            INSERT INTO national_team_h2h_cache (
                pair_key, home_team_id, away_team_id, meetings_used,
                meetings_json, national_h2h_score, detail_json, source, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(pair_key) DO UPDATE SET
                ...
                national_h2h_score = excluded.national_h2h_score,
            ...
            (
                ...
                home_team_id,
                away_team_id,
                int(payload.get("meetings_used") or 0),
                json.dumps(payload.get("meetings"), ensure_ascii=False),
                payload.get("national_h2h_score"),
    ...
    def get_national_h2h_cache(
        ...
        pair_key = f"{min(home_team_id, away_team_id)}-{max(home_team_id, away_team_id)}"
        row = self._conn.execute(
            "SELECT * FROM national_team_h2h_cache WHERE pair_key = ?",
            (pair_key,),
        ).fetchone()
        if not row:
            return None
        out = dict(row)
        for key, plain in (("meetings_json", "meetings"), ("detail_json", "detail")):
            raw = out.pop(key, None)
            if raw:
                try:
                    out[plain] = json.loads(raw)
    ...
    def count_national_form_cache(self) -> int:
        row = self._conn.execute("SELECT COUNT(*) AS c FROM national_team_form_cache").fetchone()
        return int(row["c"]) if row else 0

    def count_national_h2h_cache(self) -> int:
        row = self._conn.execute("SELECT COUNT(*) AS c FROM national_team_h2h_cache").fetchone()
        return int(row["c"]) if row else 0
    ...
    def invalidate_worldcup_stored_prediction(
        self,
        fixture_id: int,
        *,
        reason: str,
        ...
        cur = self._conn.execute(
            ...
            WHERE fixture_id = ?
            """,
            (now, reason, superseded_by, now, int(fixture_id)),
        )
        self._conn.commit()
    ...
    def list_worldcup_stored_prediction_rows(
        ...
        *,
        ...
        query = "SELECT * FROM worldcup_stored_predictions WHERE competition_key = ?"
        if not include_inactive:
            query += " AND (is_active IS NULL OR is_active = 1)"
        rows = self._conn.execute(query, (competition_key,)).fetchall()
    ...
    def list_worldcup_prediction_evaluations(
        ...
        *,
        ...
        rows = self._conn.execute(
            "SELECT * FROM worldcup_prediction_evaluations WHERE competition_key = ?",
            (competition_key,),
        ).fetchall()
        out = []
        for row in rows:
            d = dict(row)
            d["overall_status"] = d.get("overall_status")
            d["safe_pick_status"] = d.get("safe_pick_status")
            d["value_pick_status"] = d.get("value_pick_status")
            d["aggressive_pick_status"] = d.get("aggressive_pick_status")
            d["market_1x2_status"] = d.get("market_1x2_status")
            d["market_ou_status"] = d.get("market_ou_status")
            d["market_btts_status"] = d.get("market_btts_status")
            d["market_dc_status"] = d.get("market_dc_status")
            d["no_bet"] = bool(d.get("no_bet"))
            out.append(d)
        return out

    def list_worldcup_prediction_evaluations_filtered(
        self,
        *,
        competition_key: str = "world_cup_2026",
        ...
        query = """
            SELECT e.* FROM worldcup_prediction_evaluations e
            LEFT JOIN worldcup_stored_predictions s ON s.fixture_id = e.fixture_id
            WHERE e.competition_key = ?
        """
        params: list[Any] = [competition_key]

        if fixture_id is not None:
            query += " AND e.fixture_id = ?"
            params.append(int(fixture_id))

        sf = str(status_filter or "all").lower()
        if sf == "correct":
            query += " AND e.overall_status = 'correct'"
        elif sf == "wrong":
            query += " AND e.overall_status = 'wrong'"
        elif sf == "pending":
            query += " AND e.overall_status = 'pending'"
        elif sf in {"unknown", "void"}:
            query += " AND e.overall_status IN ('unknown', 'void')"

        ptf = str(pick_tier_filter or "all").lower()
        if ptf == "official":
            query += " AND e.no_bet = 0"
        elif ptf == "caution":
            query += " AND e.no_bet = 1"

        if confidence_min is not None:
            query += " AND CAST(json_extract(s.payload_json, '$.confidence') AS REAL) >= ?"
    ...
    def count_worldcup_prediction_evaluations_filtered(
        self,
        *,
        competition_key: str = "world_cup_2026",
        fixture_id: int | None = None,
        ...
        query = """
            SELECT COUNT(*) AS c FROM worldcup_prediction_evaluations e
            LEFT JOIN worldcup_stored_predictions s ON s.fixture_id = e.fixture_id
            WHERE e.competition_key = ?
        """
        params: list[Any] = [competition_key]
    ...
    def insert_learning_report(
        ...
        *,
        ...
        cur = self._conn.execute(
            """
            INSERT INTO learning_reports (competition_key, report_type, payload_json, created_at)
            VALUES (?, ?, ?, ?)
            """,
            (competition_key, report_type, json.dumps(payload, ensure_ascii=False, default=str), now),
        )
        self._conn.commit()
    ...
    def list_learning_reports(
        ...
        *,
        ...
        rows = self._conn.execute(
            """
            SELECT id, competition_key, report_type, payload_json, created_at
    ...
    def get_learning_report(self, report_id: int) -> dict[str, Any] | None:
        row = self._conn.execute(
            "SELECT id, competition_key, report_type, payload_json, created_at FROM learning_reports WHERE id = ?",
            (int(report_id),),
        ).fetchone()
        if not row:
            return None
