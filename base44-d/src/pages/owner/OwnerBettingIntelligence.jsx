        ...
        <tbody>
          {rows.map((r) => (
            <tr key={`${r.snapshot_id}-${r.market_id}`} className="border-t border-white/5">
              ...
              <td className="py-2 pr-2">{r.model_probability != null ? `${(r.model_probability * 100).toFixed(1)}%` : "—"}</td>
              <td className="py-2 pr-2">{r.implied_probability != null ? `${(r.implied_probability * 100).toFixed(1)}%` : "—"}</td>
              <td className="py-2 pr-2">{r.edge != null ? `${(r.edge * 100).toFixed(1)}%` : "—"}</td>
              <td className="py-2 pr-2">{r.ev != null ? r.ev.toFixed(3) : "—"}</td>
              <td className="py-2 pr-2">{r.kelly_capped != null ? `${(r.kelly_capped * 100).toFixed(2)}%` : "—"}</td>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
        ...
        <IntelligenceCard><p className="text-[#94A3B8] text-xs">No bet</p><p className="text-xl font-bold">{summary.no_bet ?? 0}</p></IntelligenceCard>
      </div>
